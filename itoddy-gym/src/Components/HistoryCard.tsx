...
import { DataHistoryDTO } from "@dtos/DataHistoryDTO";

type HistoryCardProps = {
    history: DataHistoryDTO
}
    ...
    return (
        <HStack w='full' px={5} py={4} mb={3} bg='gray.600' rounded='md' alignItems='center' justifyContent='space-between'>
            <VStack mr={5} flex={1}>
                ...
                <Text color='gray.100' fontSize='lg' numberOfLines={1}>{history.name}</Text>
            </VStack>

            <Text color='gray.300' fontSize='md'>{history.hour}</Text>
        </HStack>
    )
}